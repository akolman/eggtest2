...
namespace EggDotNet.Encryption.Zip
{
	internal sealed class ZipDecryptStream : Stream
	{
		...
		private uint DecryptByte()
		{
			var res = (_keys[2] | 2) & 0xFFFF;
			return ((res * (res ^ 1)) >> 8) & 0xFF;
		}

		private void UpdateKeys(uint data)
		{
			data &= 0xFF;
			_keys[0] = gcrcTable[(_keys[0] ^ data) & 0xFF] ^ (_keys[0] >> 8);
			_keys[1] = _keys[1] + (_keys[0] & 0xFF);
			_keys[1] = _keys[1] * 134775813 + 1;
			_keys[2] = gcrcTable[(_keys[2] ^ (_keys[1] >> 24)) & 0xFF] ^ ((_keys[2] >> 8) & 0x00FFFFFF);
		}
