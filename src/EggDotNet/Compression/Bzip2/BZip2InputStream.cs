	...
	internal class BZip2InputStream : Stream
	{
		...
		private void GetAndMoveToFrontDecode()
		{
			...
			while (zvec > limit[zt][zn])
			{
				if (zn > 20)
				{ // the longest code
					throw new System.Exception("Bzip data error");
				}
				zn++;
				while (bsLive < 1)
				{
		...
		private void SetDecompressStructureSizes(int newSize100k)
		{
			...
			{
				throw new System.Exception("Invalid block size");
			}
		...
		{
			throw new System.Exception("BZip2 input stream block overrun");
		}

		private static void BadBlockHeader()
		{
			throw new System.Exception("BZip2 input stream bad block header");
		}

		private static void CrcError()
		{
			throw new System.Exception("BZip2 input stream crc error");
		}
